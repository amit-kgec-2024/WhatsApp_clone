    ...
    <div className="profile-animation w-full bg-dark6 h-screen">
      <div className={`${activeDisappearing ? "hidden" : ""}`}>
        ...
        <h1 className="w-full text-sm text-center py-8">
          Update your{" "}
          <button onClick={()=> handelDisappearingClick('disappearing')} className="text-teal-400">
            default messages timer
          </button>{" "}
          in Settings
        </h1>
      </div>
      {activeDisappearing === 'disappearing' && <div className="">
        <div className="bg-dark3 p-4 pl-6 pt-14 flex flex-row items-center gap-5">
          <button onClick={()=> setActiveDisappearing(false)} className="text-lg">
            <FaArrowLeft />
          </button>
        ...
        <p className="text-sm text-slate-400 px-8 py-4">
          When turned on, all new individual chats will start with disappearing
          messages set to the duration you select. This setting will not affect
          your existing chats.
        </p>
        <Link
          onClick={handleButtonClick}
